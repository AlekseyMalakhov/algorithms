...
promise resolves:
    ...
    all the resolved values of promises in the same order as they were in the functions. The promise should resolve when all the asynchronous 
...
var promiseAll = function (functions) {
    return new Promise((resolve, reject) => {
        let calls = 0;
        const res = [];
        const updateRes = (val, i) => {
            calls++;
            res[i] = val;
            if (calls === functions.length) {
                resolve(res);
            }
        };
        ...
        for (let i = 0; i < functions.length; i++) {
            functions[i]()
                .then((result) => updateRes(result, i))
...
const functions1 = [
    ...
    //() => new Promise((resolve, reject) => setTimeout(() => reject("dlsfsdf"), 300)),
];
