...
var findPoisonedDuration = function (timeSeries, duration) {
    ...
    for (let i = 0; i < timeSeries.length; i++) {
        if (i !== timeSeries.length - 1) {
            const durationToTheNextAttack = timeSeries[i + 1] - timeSeries[i];
            if (durationToTheNextAttack > duration) {
                res = res + duration;
            } else {
                res = res + durationToTheNextAttack;
            }
        } else {
            res = res + duration;
        }
...
console.log(findPoisonedDuration([7], 2));
