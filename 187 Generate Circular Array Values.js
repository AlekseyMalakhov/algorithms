...
Each subsequent time gen.next() is called, an integer jump will be passed into the function (Ex: gen.next(-3)).
    ...
    If jump is negative, the index should decrease by the magnitude of that value, however if the current index is the first index, 
    it should instead jump to the last index.
...
var cycleGenerator = function* (arr, startIndex) {
    ...
    while (true) {
        ...
        if (jump > 0) {
            //go positive
            current = (jump % arr.length) + current;
        ...
        if (jump < 0) {
            //go negative
            current = current + (jump % arr.length);
            if (current < 0) {
                current = arr.length + current;
            }
        }
    }
};
...
console.log(gen.next(1).value); // 2
console.log("------------------------");
console.log(gen.next(4).value); // 4
console.log("------------------------");
console.log(gen.next(0).value); // 5
console.log("------------------------");
console.log(gen.next(-1).value); // 5
console.log("------------------------");
