...
class EventEmitter {
    ...
    subscribe(eventName, callback) {
        if (!this.state.has(eventName)) {
            this.state.set(eventName, []);
        }
        const arr = this.state.get(eventName);
        arr.push(callback);
        this.state.set(eventName, arr);
        return {
            unsubscribe: () => {
                const arr = this.state.get(eventName);
                const newArr = arr.filter((item) => item !== callback);
                if (newArr.length === 0) {
                    this.state.delete(eventName);
                } else {
                    this.state.set(eventName, newArr);
                }
            },
        };
    }
...
const emitter = new EventEmitter();
const subscription1 = emitter.subscribe("lalal", (arg) => arg);
const subscription2 = emitter.subscribe("lalal", (arg) => arg + "heheh");
const subscription3 = emitter.subscribe("didid", (arg) => arg + "hohoh");
console.log(emitter.state);
// console.log(subscription1.unsubscribe());
// console.log(emitter.state);
// console.log(subscription3.unsubscribe());
// console.log(emitter.state);
console.log(emitter.emit("didid", ["kekek"]));
