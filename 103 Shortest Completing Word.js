...
var shortestCompletingWord = function (licensePlate, words) {
    ...
    const letters = licensePlate.match(/[A-Za-z]/g);
    const lettersLC = letters.map((item) => item.toLowerCase());

    for (let i = 0; i < lettersLC.length; i++) {
        const char = lettersLC[i];
    ...
    const checkWord = (word) => {
        ...
        for (x in obj) {
            if (!objCheck[x] || objCheck[x] < obj[x]) {
                return false;
            }
        }
        return true;
...
var shortestCompletingWord = function (licensePlate, words) {
    let c = licensePlate.replace(/[\d\s]/g, "").toLowerCase();
