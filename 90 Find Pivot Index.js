...
var pivotIndex = function (nums) {
    let lastSumLeft = null;
    let lastSumRight = null;

    for (let i = 0; i < nums.length; i++) {
        if (lastSumLeft === null) {
            lastSumLeft = 0;
        } else {
            lastSumLeft = lastSumLeft + nums[i - 1];
        }
        if (lastSumRight === null) {
            lastSumRight = nums.reduce((acc, val) => acc + val) - nums[0];
        } else {
            lastSumRight = lastSumRight - nums[i];
        }
