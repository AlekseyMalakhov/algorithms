...
buy one lemonade and pay with either a $5, $10, or $20 bill. You must provide the correct change to each 
...
var lemonadeChange = function (bills) {
    ...
    const checkChange = (cash) => {
        ...
        //cash = 20
        if (change === 15) {
            if (!countChange([10, 5]) && !countChange([5, 5, 5])) {
                return false;
            }
        }
        return true;
    ...
    for (let i = 0; i < bills.length; i++) {
        //check money and try to give change. If can't give change - return false
        if (!checkChange(bills[i])) {
            return false;
        }
